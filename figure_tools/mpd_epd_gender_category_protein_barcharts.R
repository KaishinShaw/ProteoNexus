...
# -------------------------------------------------------------------
# 2.1 Extract protein column (unique values) for each dataset
# -------------------------------------------------------------------
mpd_proteins <- mpd %>% 
    pull(protein) %>% 
    unique()
...
# If desired, inspect:
# length(mpd_proteins); length(epd_proteins)

# -------------------------------------------------------------------
# 3. Pre-process: factor levels, deduplication, and color schemes
# -------------------------------------------------------------------
...
# Remove duplicate proteins within each gender × Category
mpd <- mpd %>%
    distinct(gender, Category, protein, .keep_all = TRUE)
