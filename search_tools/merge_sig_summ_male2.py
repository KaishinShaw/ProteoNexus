...
# -*- coding: utf-8 -*-
"""
merge_sig_summ_male2.py

This script searches for all `sig_summ_male2.assoc.tsv` files under the
directory structure:
...
Usage:
    python merge_sig_summ_male2.py
    python merge_sig_summ_male2.py \
        --input-dir /path/to/pqtl \
        --output-csv merged_male2.csv \
        --output-pkl merged_male2.pkl
"""
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Merge multiple sig_summ_male2.assoc.tsv files into one DataFrame."
    )
    parser.add_argument(
        "--input-dir", "-i",
    ...
    parser.add_argument(
        "--output-csv", "-c",
        default="merged_sig_summ_male2.csv",
        help="Path to write the merged CSV file."
    )
    parser.add_argument(
        "--output-pkl", "-p",
        default="merged_sig_summ_male2.pkl",
...
def find_assoc_files(input_dir: str) -> list:
    """
    Locate all sig_summ_male2.assoc.tsv files under the given directory.
    """
    pattern = os.path.join(input_dir, "*", "output", "sig_summ_male2.assoc.tsv")
...
def main():
    ...
    logging.info(f"Searching for 'sig_summ_male2.assoc.tsv' under: {args.input_dir}")
