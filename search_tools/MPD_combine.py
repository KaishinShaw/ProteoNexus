#!/usr/bin/env python3
"""
Concatenate all TSV files under /gpfs/chencao/ysbioinfor/project/proteohubProject/web_out/M-P-D/,
and prepend the following four columns (in this order):

 1. modules : fixed value "M-P-D"
 2. PHM     : the immediate parent directory name ({PHM number})
...
# Configuration
ROOT_DIR    = Path("/gpfs/chencao/ysbioinfor/project/proteohubProject/web_out/M-P-D/")
TSV_PATTERN = "**/*.tsv"                # recursive glob pattern
NEW_COLS    = ["modules", "PHM", "PHD", "gender"]
...
for tsv_file in ROOT_DIR.glob(TSV_PATTERN):
    ...
    df.insert(0, "modules", "M-P-D")
    df.insert(1, "PHM",      phm_code)
    df.insert(2, "PHD",      phd)
...
output_path = ROOT_DIR / "M-P-D_merged.tsv"
combined_df.to_csv('M-P-D_merged.tsv', sep="\t", index=False)