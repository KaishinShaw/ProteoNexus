#!/usr/bin/env python3
"""
Concatenate all TSV files under /gpfs/chencao/ysbioinfor/project/proteohubProject/web_out/G-P-D/,
and prepend the following four columns (in this order):

 1. modules : fixed value "G-P-D"
 2. rsID     : the immediate parent directory name ({rs number})
...
# Configuration
ROOT_DIR    = Path("/gpfs/chencao/ysbioinfor/project/proteohubProject/web_out/G-P-D/")
TSV_PATTERN = "**/*.tsv"                # recursive glob pattern
NEW_COLS    = ["modules", "rsID", "PHD", "gender"]
...
for tsv_file in ROOT_DIR.glob(TSV_PATTERN):
    ...
    df.insert(0, "modules", "G-P-D")
    df.insert(1, "rsID",      phm_code)
    df.insert(2, "PHD",      phd)
...
output_path = ROOT_DIR / "G-P-D_merged.tsv"
combined_df.to_csv('G-P-D_merged.tsv', sep="\t", index=False)