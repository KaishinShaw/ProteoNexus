...
args_list = list(
  ...
  make_option("--group", type = "integer", default=NULL,
              help = "INPUT: disease id", metavar = "character")
)
opt_parser = OptionParser(option_list=args_list)
opt = parse_args(opt_parser)

# opt = list(sex = "male", pair = 1, group = 3)
...
# Load data
## Sig. emd
sig_emd <- fread(paste0(DATA_PATH, "sig_total/im_emd_", opt$sex, ".txt"), header = F)
# sig_emd$group <- c(rep(1: floor(nrow(sig_emd)/1000), each = 1000), 
#                    rep(ceiling(nrow(sig_emd)/1000), nrow(sig_emd)%%1000))
# sig_emd <- sig_emd %>% filter (group == opt$group)
